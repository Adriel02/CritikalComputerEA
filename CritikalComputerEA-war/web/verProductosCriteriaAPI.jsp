...
<html>
    ...
    <body>
        <table>
            <tr><th>Nombre</th><th>Precio</th><th>PrecioDescuento</th><th>Descripcion</th>><th>Proveedor</th>><th>Oferta</th><th>Eliminar</th></tr>
                    <% for (int i = 0; i < productos.size(); i++) {%>
            <tr>
            <td><%= productos.get(i).getNombre()%></td>
            <td><%= productos.get(i).getPrecio()%></td>
            <td><%= productos.get(i).getPrecio()*(mapaOfertas.get(productos.get(i).getOferta()).getDescuento())/100%></td>
            ...
            <td><a href="/CritikalComputerEA-war/frontServlet?command=AccionesProducto.deleteProducto&id=<%=productos.get(i).getId()%>">Eliminar</a></td>
            <td><a href="/CritikalComputerEA-war/frontServlet?command=AccionesProducto.modificarProducto&id=<%=productos.get(i).getId()%>">Modificar</a></td>
            <%} else {%>
            <td><a href="/CritikalComputerEA-war/frontServlet?command=AccionesCarrito.addCarrito&id=<%=productos.get(i).getId()%>">Añadir al carrito</a></td>
            </tr>
            <%}%>
            <%}%>
        </table>   
